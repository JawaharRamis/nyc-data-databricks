...
class Config():    
    def __init__(self):      
        ...
        self.unmanaged_loc = spark.sql("describe external location `unmanaged-location`").select("url").collect()[0][0]
        self.managed_loc = spark.sql("describe external location `managed-location`").select("url").collect()[0][0]
        self.db_name = "nyc_collision"
        self.catalog = "nyc_collision"
